...
% Initialize arrays for motor power, vehicle speed, and efficiency
vehicle_speed = zeros(length(motor_speed), length(motor_torque)); % Initialize vehicle speed array
motor_power = zeros(length(motor_speed), length(motor_torque)); % Initialize motor power array
...
for g = 1:length(gear_ratios)
    for i = 1:length(motor_speed)
        for j = 1:length(motor_torque)
            ...
            motor_power(i,j) = P_motor_in; % Store motor power
            ...
            % For simplicity, this uses an example formula combining rolling resistance and aero power
...
% Plot Efficiency vs Power for each gear
figure;
hold on;

% Plot efficiency for each gear
for g = 1:length(gear_ratios)
    % Extract efficiency values for the current gear
    efficiency_flat = efficiency_per_gear(:,:,g); 

    % Flatten motor_power and efficiency_per_gear for plotting
    power_flat = motor_power(:); % Flatten motor power array
    efficiency_flat = efficiency_flat(:); % Flatten the efficiency array for the current gear
    
    % Plot efficiency vs motor power for each gear
    plot(power_flat, efficiency_flat, 'DisplayName', sprintf('Gear %d', g));
end

hold off;

xlabel('Motor Power (W)');
ylabel('Efficiency');
title('Efficiency vs Motor Power for Each Gear');
legend('show');
